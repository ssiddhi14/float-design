...
function LoginPage() {
  ...
  const handleGoogleAuth = () => {
    ...
    signInWithGoogle(
      (googleUser) => {
        ...
        if (googleUser.email.trim().toLowerCase() === "admin@1234") {
          nav({ to: "/admin" });
        } else {
          nav({ to: "/" });
        }
  ...
  useEffect(() => {
    ...
    if (existingUser && existingUser.email.trim().toLowerCase() === "admin@1234") {
      nav({ to: "/admin" });
    }
  }, [nav]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check Admin credentials first (works in both Login & Signup tabs)
    if (trimmedEmail.toLowerCase() === "admin@1234" || isAdmin(trimmedEmail, trimmedPass)) {
      if (isAdmin(trimmedEmail, trimmedPass)) {
        setUser({ firstName: "Admin", lastName: "", email: "admin@1234" });
        window.dispatchEvent(new Event("auth-updated"));
        nav({ to: "/admin" });
        return;
      } else {
    ...
    if (mode === "signup") {
      ...
      if (!trimmedEmail.includes("@")) { setError("Please enter a valid email."); return; }
      if (trimmedPass.length < 6) { setError("Password must be at least 6 characters."); return; }
      if (pass !== confirm) { setError("Passwords do not match."); return; }
      setUser({ firstName: first.trim(), lastName: last.trim(), email: trimmedEmail });
    } else {
      if (!trimmedEmail.includes("@") || !trimmedPass) { setError("Please enter your email and password."); return; }
      const name = trimmedEmail.split("@")[0];
      setUser({ firstName: name, lastName: "", email: trimmedEmail });
    }
        ...
        <div style={{ width: "100%", maxWidth: 440 }}>
          ...
          <div style={{ background: "var(--card-bg)", backdropFilter: "blur(24px)", border: "1px solid var(--card-border)", borderRadius: 20, padding: "32px 28px", boxShadow: "0 8px 48px rgba(0,0,0,0.06)" }}>
            ...
            <button type="button"
              ...
              style={{ width: "100%", padding: "13px 0", background: "var(--card-bg)", color: "var(--text-primary)", border: "1px solid var(--card-border)", borderRadius: 12, fontSize: 12, fontWeight: 500, letterSpacing: "0.06em", cursor: googleLoading ? "wait" : "pointer", fontFamily: "'Plus Jakarta Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, opacity: googleLoading ? 0.7 : 1, transition: "all 0.2s ease" }}>
